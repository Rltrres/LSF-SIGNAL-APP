...
ARCHETYPES = [
    ...
    "MidSession_Internal",
    "HTF_POI_Sweep",
    "NY_Low_Reversal",  # <-- new explicit model
]
...
SWEEP_PROFILES: Dict[str, Dict[str, Any]] = {
  ...
  "NY_Low_Reversal": {
      "bias_mode": "reversal",
      "adx_min": 20, "post_sweep_delay": 3,
      "require_vwap_flip": True,
      "expected_vwap": "reclaim",
      "grade_weights": {"sweep":30,"mss":20,"vwap":20,"adx":20,"bias":10},
      "notes": "NY sweeps the London (or prior) Low, then reclaims VWAP and reverses LONG."
  }
}
...
def auto_model_from_context(session:Session, sweep_type:str, desired:Dir) -> Optional[str]:
    """Return a recommended model based on who swept whom."""
    if session=="NY" and sweep_type in ("London_Low","PDL","Midnight_Low","Settlement_Low") and desired=="LONG":
        return "NY_Low_Reversal"
    if session=="NY" and sweep_type in ("London_High","PDH","Midnight_High","Settlement_High") and desired=="SHORT":
        return "London_High_Reversal"
    if session=="NY" and sweep_type in ("Asia_Low","Asia_High") and desired in ("LONG","SHORT"):
        return "Asia_London_NY_Continuation"
    return None

def evaluate_signal(desired:Dir, inp:Inputs, profiles:Dict[str,Dict[str,Any]]|None=None):
    profiles = profiles or SWEEP_PROFILES

    # auto-switch model if context suggests it
    ...
    banners = []
    if rec: banners.append(f"Auto-switched model → {rec}")
    ...
    return {
        "model_used": model_name, "entry_ready": entry_ready, "grade": grade,
        "components": {
