# signal_engine_v3_11.py
from __future__ import annotations
from dataclasses import dataclass
from typing import Literal, Optional, Dict, Any, Tuple
import os, csv, json, datetime as dt, copy
...
_SWEEP_PROFILES_DEFAULT: Dict[str, Dict[str, Any]] = {
  "Asia_London_NY_Continuation": {"bias_mode":"continuation","adx_min":24,"post_sweep_delay":3,"require_vwap_flip":False,"expected_vwap":"support","grade_weights":{"sweep":30,"mss":20,"vwap":20,"adx":15,"bias":15},"notes":"Asia/London sweep -> NY continuation. VWAP support; ADX>~24; delay≈3."},
  "London_High_Reversal": {"bias_mode":"reversal","adx_min":25,"post_sweep_delay":4,"require_vwap_flip":True,"expected_vwap":"resistance","grade_weights":{"sweep":25,"mss":20,"vwap":25,"adx":15,"bias":15},"notes":"London premium -> NY fade. VWAP rejection; bearish MSS."},
  "PDH_PDL_Trap": {"bias_mode":"reversal","adx_min":27,"post_sweep_delay":2,"require_vwap_flip":True,"expected_vwap":"flip","grade_weights":{"sweep":35,"mss":25,"vwap":15,"adx":10,"bias":15},"notes":"Prior Day extreme trap; flip back through VWAP + MSS."},
  "MidSession_Internal": {"bias_mode":"reversal","adx_min":24,"post_sweep_delay":1,"require_vwap_flip":True,"expected_vwap":"flip","grade_weights":{"sweep":25,"mss":20,"vwap":20,"adx":20,"bias":15},"notes":"IB high/low sweep 10:15–10:45 ET; micro-FVG + VWAP flip + MSS."},
  "HTF_POI_Sweep": {"bias_mode":"reversal","adx_min":20,"post_sweep_delay":4,"require_vwap_flip":True,"expected_vwap":"reclaim","grade_weights":{"sweep":30,"mss":15,"vwap":20,"adx":20,"bias":15},"notes":"HTF POI sweep (1H/4H). VWAP reclaim + structure shift."},
  "NY_Low_Reversal": {"bias_mode":"reversal","adx_min":20,"post_sweep_delay":3,"require_vwap_flip":True,"expected_vwap":"reclaim","grade_weights":{"sweep":30,"mss":20,"vwap":20,"adx":20,"bias":10},"notes":"NY sweeps the London (or prior) Low, reclaims VWAP, then reverses LONG."}
}
SWEEP_PROFILES = copy.deepcopy(_SWEEP_PROFILES_DEFAULT)

def defaults_for(model:str)->Dict[str,Any]: return copy.deepcopy(_SWEEP_PROFILES_DEFAULT[model])
def reset_model_to_defaults(model:str): SWEEP_PROFILES[model] = defaults_for(model)

def load_profiles_from_excel(df):
    if df is None or df.empty: return
    for _, row in df.iterrows():
        arche = row.get("Archetype")
        if not isinstance(arche, str) or arche not in SWEEP_PROFILES: continue
...
def load_profiles_from_json(path:str):
    ...
    for k,v in data.items():
        if k in SWEEP_PROFILES and isinstance(v, dict): SWEEP_PROFILES[k].update(v)
...
class Inputs:
    ...
    bias_logic_mode: Literal["strict","adaptive","auto"] = "auto"
    vwap_close_confirm: bool = True

def vwap_gate(expected, side, slope, need_close)->Tuple[bool,str]:
    if expected=="support":
        return (side=="ABOVE"), "support"
    if expected=="resistance":
        return (side=="BELOW"), "resistance"
    if expected in ("flip","reclaim"):
        return (slope in ("UP","DOWN")) and (side!="TOUCHING"), "flip/reclaim"
    return True, "any"

def bias_gate_strict(htf,l15,l3,mode,desired)->bool:
...
def bias_gate_adaptive(htf,l15,l3,mode,desired,vwap_ok,adx_ok)->Tuple[bool,str]:
    if bias_gate_strict(htf,l15,l3,mode,desired):
        return True, "strict"
    one_ltf = ( (desired=="LONG" and (l15=="BULL" or l3=="BULL")) or
                (desired=="SHORT" and (l15=="BEAR" or l3=="BEAR")) )
    if one_ltf and vwap_ok and adx_ok:
...
def evaluate_signal(desired, inp:Inputs, profiles=None):
    ...
    if prof.get("require_vwap_flip", True):
        vwap_ok, vwhy = vwap_gate(prof.get("expected_vwap","support"), inp.vwap_side, inp.vwap_slope, inp.vwap_close_confirm)
    else:
        vwap_ok, vwhy = True, "not-required"
    mode = inp.bias_logic_mode
    if mode=="auto":
        mode = "adaptive" if prof.get("bias_mode")=="reversal" else "strict"
    if mode=="strict":
        bias_ok = bias_gate_strict(inp.htf_60m_bias, inp.ltf_15m_bias, inp.ltf_3m_bias, prof.get("bias_mode","continuation"), desired)
        bias_note = "strict"
    else:
        bias_ok, bias_note = bias_gate_adaptive(inp.htf_60m_bias, inp.ltf_15m_bias, inp.ltf_3m_bias, prof.get("bias_mode","continuation"), desired, vwap_ok, adx_ok)
    ...
    tag = "TRANSITIONAL_REVERSAL" if (bias_note=="transitional") else ("CONTINUATION" if prof.get("bias_mode")=="continuation" else "REVERSAL")
    return {"model_used": model_name, "entry_ready": entry_ready, "grade": grade, "tag": tag,
            "components": {"delay_ok":delay_ok,"mss_ok":mss_ok,"vwap_ok":vwap_ok,"adx_ok":adx_ok,
                           ...
                           "bias_note":bias_note,"vwap_why":vwhy},
            "profile": prof}

def log_signal_csv(path, model, desired, res, inp:Inputs):
    dirp = os.path.dirname(path)
    if dirp and not os.path.exists(dirp): os.makedirs(dirp, exist_ok=True)
    headers = ["timestamp","session","model","sweep_type","direction","grade","entry_ready","tag",
               "adx_now","adx_slope","vwap_side","vwap_slope","htf","l15","l3","bias_note"]
    row = {"timestamp": dt.datetime.utcnow().isoformat(),
        ...
        "grade": res.get("grade"), "entry_ready": res.get("entry_ready"), "tag": res.get("tag"),
        ...
        "bias_note": res["components"].get("bias_note","")}
